...
public class SplitShellUtils {
	...
	private static void spliteAggregateShell(ShellMethod method, String shell) {
		//aggregate method
		if (Constants.METHOD_TYPE_AGGREGATE.equalsIgnoreCase(method.getMethodType())) {
			int start = shell.indexOf(Constants.SPLITE_START);
			
			int end = shell.indexOf(Constants.SPLITE_END);
			...
			if (start >= 0 && end >= 0) {
				String pipeShell = shell.substring(start + Constants.SPLITE_START.length(), end);
				
				String[] pipes = pipeShell.split(Constants.PIPE_SPLITE);

				for (int i = 0;i < pipes.length;i++) {
					
					pipes[i] = "{" + pipes[i] + "}";
					
					int pre = pipes[i].indexOf(Constants.PLACE_HOLDER_PRE);
					int suf = pipes[i].indexOf(Constants.PLACE_HOLDER_SUF);
